...
sensor_ht = HTU21()
sleep(0.1)
intervalo_sensores = 5
guardar_sensores = False
dt_inicio = 0
dt_fin = 0
...
class Ui_ConfigurarPantalla(object):
    ...
    def setupUi(self, Dialog):
        ...
        self.horizontalLayout_99.addWidget(self.label_99)
        self.le_t_sensor = QtWidgets.QSpinBox(self.verticalLayoutWidget)
        self.le_t_sensor.setRange(5, 3600) 
        self.le_t_sensor.setObjectName("le_t_sensor")
        self.horizontalLayout_99.addWidget(self.le_t_sensor)
        self.verticalLayout.addLayout(self.horizontalLayout_99)
        self.label_98 = QtWidgets.QLabel(self.verticalLayoutWidget)
        self.label_98.setObjectName("label_98")
        self.horizontalLayout_99.addWidget(self.label_98)
    ...
    def cargar_configs(self):
        ...
        self.le_t_sensor.setValue(intervalo_sensores)
    ...
    def guardar_configs(self):
        ...
        global intervalo_sensores, dt_inicio, dt_fin
        intervalo_sensores = self.le_t_sensor.value()
        dt_inicio = tiempo['fh_inicio']
        dt_fin = tiempo['fh_inicio'] + dt.timedelta(
            seconds=tiempo['duracion_videos'].second*tiempo['cantidad_videos'],
            minutes=tiempo['duracion_videos'].minute*tiempo['cantidad_videos'],
            hours=tiempo['duracion_videos'].hour*tiempo['cantidad_videos'])
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label_99.setText(_translate("Dialog", "Intervalo Sensado:"))
        self.label_98.setText(_translate("Dialog", "s"))
...
class Ui_MainWindow(QMainWindow):
    def setupUi(self, MainWindow):
        ...
        def hilo_sensado():
            ...
            t_record=1
            formato="yyyyMMdd"
            formato_2="yyyy/MM/dd-hh:mm:ss"

            while True:
                s_Luz = str(format(sensor_luz.read_light(), '.2f'))
                sleep(0.1)
                temp = sensor_ht.read_temperature()
                s_Temperatura=format(temp, '.2f')
                hum = sensor_ht.read_humidity()
                s_Humedad=format(hum, '.2f')
                ...
                if guardar_sensores:
                    if(crear_archivo):
                        ...
                        name='log_'+fecha.toString(formato)+'.txt'
                    if((start.secsTo(QDateTime.currentDateTime()) > (intervalo_sensores-2)) and
                       (dt.datetime.today() > dt_inicio) and
                       (dt.datetime.today() < dt_fin)):
                        try:
                            archivo = open(name,'a')
                        except:
                            archivo = open(name,'w')
                        ...
                        archivo.write(QDateTime.currentDateTime().toString(formato_2)+"\n")
                        archivo.close()
                        print('guardando sensados '+QDateTime.currentDateTime().toString(formato_2))
